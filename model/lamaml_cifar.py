...
from model.lamaml_base import BaseNet
...
class Net(BaseNet):
    ...
    def inner_update(self, x, fast_weights, y, t):
        ...
        for i in range(len(grads)):
            grads[i] = torch.clamp(
                grads[i], min=-self.args.grad_clip_norm, max=self.args.grad_clip_norm
            )

        fast_weights = list(
            map(
                lambda p: p[1][0] - p[0] * p[1][1], zip(grads, zip(fast_weights, self.net.alpha_lr))
            )
        )
